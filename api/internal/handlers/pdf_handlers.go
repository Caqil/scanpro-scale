...
func (h *PDFHandler) RemovePagesFromPDF(c *gin.Context) {
	// Get operation type and user ID from context
	operation := "remove"
	userID, _ := c.Get("userId")
	...
	pagesToRemoveStr := c.PostForm("pagesToRemove")
	if pagesToRemoveStr == "" {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "No pages to remove specified",
		})
		return
	}

	var pagesToRemove []int
	if err := json.Unmarshal([]byte(pagesToRemoveStr), &pagesToRemove); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "Invalid pages format: " + err.Error(),
		})
		return
	}

	// Create temp file from uploaded file
	tempFile, err := os.CreateTemp(h.config.TempDir, "upload-*.pdf")
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
	...
	// Count the number of pages in the PDF using qpdf
	pageCountCmd := exec.Command("qpdf", "--show-npages", tempFile.Name())
	pageCountOutput, err := pageCountCmd.CombinedOutput()
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Failed to count PDF pages: " + err.Error() + " - " + string(pageCountOutput),
		})
		return
	}

	// Parse the page count output
	...
	// Charge user for operation if needed (similar to other operations)
	if userID != nil {
		result, err := h.balanceService.ProcessOperation(userID.(string), operation)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{
				"error": "Failed to process operation charge: " + err.Error(),
			})
			return
		}
		...
		if !result.Success {
			c.JSON(http.StatusPaymentRequired, gin.H{
				"error":                   result.Error,
				"insufficientBalance":     !result.UsedFreeOperation && result.Error == "Insufficient balance",
				"freeOperationsRemaining": result.FreeOperationsRemaining,
				"currentBalance":          result.CurrentBalance,
			})
			return
	...
	outputFilename := strings.TrimSuffix(header.Filename, filepath.Ext(header.Filename)) + "_pages_removed.pdf"
	outputPath := filepath.Join(h.config.PublicDir, "processed", outputFilename)

	// Create the selection of pages to keep (all pages except those to be removed)
	pagesToKeep := make([]string, 0, totalPages-len(pagesToRemove))
	pagesToRemoveMap := make(map[int]bool)

	for _, page := range pagesToRemove {
		pagesToRemoveMap[page] = true
	}

	for i := 1; i <= totalPages; i++ {
		if !pagesToRemoveMap[i] {
			pagesToKeep = append(pagesToKeep, strconv.Itoa(i))
		}
	}

	// If no pages to keep, return error
	if len(pagesToKeep) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": "Cannot remove all pages from a PDF",
		})
		return
	}

	// Build the page range string for qpdf
	// qpdf expects comma-separated ranges like 1-3,5,7-9
	var rangeStrings []string
	var rangeStart int

	for i, page := range pagesToKeep {
		...
		prevPageNum, _ := strconv.Atoi(pagesToKeep[i-1])
		if pageNum == prevPageNum+1 {
			// Current page is consecutive with previous
			continue
		} else {
			// End of a range
			if rangeStart == prevPageNum {
				// Single page range
				rangeStrings = append(rangeStrings, strconv.Itoa(rangeStart))
			} else {
				// Multi-page range
				rangeStrings = append(rangeStrings, fmt.Sprintf("%d-%d", rangeStart, prevPageNum))
			}
			// Start a new range
			rangeStart = pageNum
		}
	...
	// Add the last range
	if len(pagesToKeep) > 0 {
		lastPageNum, _ := strconv.Atoi(pagesToKeep[len(pagesToKeep)-1])
		if rangeStart == lastPageNum {
			// Single page range
			rangeStrings = append(rangeStrings, strconv.Itoa(rangeStart))
		} else {
			// Multi-page range
			rangeStrings = append(rangeStrings, fmt.Sprintf("%d-%d", rangeStart, lastPageNum))
		}
	}

	// Join ranges with commas
	pageRangeStr := strings.Join(rangeStrings, ",")

	// Use qpdf to remove pages
	// qpdf input.pdf output.pdf --pages input.pdf 1,3-5,7 --
	removeCmd := exec.Command("qpdf", tempFile.Name(), outputPath, "--pages", tempFile.Name(), pageRangeStr, "--")
	removeOutput, err := removeCmd.CombinedOutput()

	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{
			"error": "Failed to remove pages: " + err.Error() + " - " + string(removeOutput),
		})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"fileUrl": "/api/file?folder=processed&filename=" + outputFilename,
	})
}
