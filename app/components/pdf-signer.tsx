...
export function PdfSigner({ initialTool = "signature" }: Props) {
  ...
  // Helper function to add field and close mobile sheet
  ...
  const handleSignatureDraw = useCallback(() => {
  ...
  }, [signatureSize, pages.length, currentPage]);
  ...
  // Generate SVG for predefined stamps with proper size and styling
  const generateStampSvg = (type: string): string => {
    ...
    // Create a more consistent SVG with explicit dimensions and styling
    return `<svg xmlns="http://www.w3.org/2000/svg" width="150" height="50" viewBox="0 0 150 50">
      ...
      <text x="75" y="30" font-family="Arial,sans-serif" font-size="16" font-weight="bold" fill="${fillColor}" text-anchor="middle" dominant-baseline="middle">${text}</text>
    </svg>`;
  };
  ...
  // Element selection, dragging, and resizing handlers
  ...
  const SignatureElementComponent = memo(
    ({
      ...
      // Render different element types appropriately
      return (
        <div
          key={element.id}
          style={elementStyles}
        ...
        >
          ...
          element.data !== "Signature Placeholder" ? (
            ...
            // Fix for SVG stamps - convert to data URL for proper rendering
            <img
              src={`data:image/svg+xml;base64,${btoa(
                unescape(encodeURIComponent(element.data))
              )}`}
              style={{
                width: "100%",
                height: "100%",
          ...
          {/* Controls for selected elements */}
          {selectedElement === element.id && (
            <>
              <button
  ...
  // Mobile tools trigger component
  const mobileToolTrigger = useMemo(
    () => (
                            ...
                            <>
                              ...
                              <SheetClose asChild>
                                <Button
                                  className="w-full h-9"
                                  onClick={() =>
                                    handleAddFieldAndCloseSheet("signature")
                                  }
                                >
                                  <CheckIcon className="h-3.5 w-3.5 mr-1.5" />
                                  Add Signature
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {[
                          ...
                          <Button
                            ...
                            style={{
                              borderColor:
                                stampType === type.id ? type.color : undefined,
                              color: type.color,
                            }}
  ...
  const renderPageThumbnails = useMemo(() => {
    return (
                          ...
                          width={
                            pages[currentPage]
                              ? Math.min(
                                  ...
                                  canvasRef.current
                                    ? Math.min(
                                        canvasRef.current.clientWidth - 40,
                                        isMobileView ? 600 : 800
                                      )
                                    : 800
                                ) * scale
                              : undefined
