...
async function repairWithQpdf(
  ...
  try {
    ...
    throw new Error(
      `Failed to repair PDF with qpdf: ${
...
async function repairWithGhostscript(
  inputPath: string,
  outputPath: string,
  ...
  try {
    ...
    const command = `${gsCommand} ${options.join(
      " "
    ...
    throw new Error(
      `Failed to repair PDF with Ghostscript: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
  }
...
async function optimizeWithPdfLib(
  ...
  try {
    ...
    if (originalPageCount !== newPageCount) {
      warnings.push(
        `Original document had ${originalPageCount} pages, new document has ${newPageCount} pages`
      );
    }
...
async function repairPasswordProtectedPdf(
  inputPath: string,
  outputPath: string,
  password: string
): Promise<{ success: boolean; details?: any }> {
  try {
    ...
    throw new Error(
      `Failed to repair password-protected PDF: ${
        error instanceof Error ? error.message : String(error)
      }`
    );
  }
