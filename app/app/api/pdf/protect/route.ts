...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!file.name.toLowerCase().endsWith(".pdf")) {
      return new NextResponse(
        JSON.stringify({ error: "Only PDF files can be protected" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
    ...
    const protectionLevel =
      (formData.get("protectionLevel") as string) || "256";

    // Restricted permissions
    const allowPrinting =
      formData.get("allowPrinting") === "true" || permission === "all";
    const allowCopying =
      formData.get("allowCopying") === "true" || permission === "all";
    const allowEditing =
      formData.get("allowEditing") === "true" || permission === "all";
    ...
    if (!protectionSuccess) {
      ...
      methodUsed = "fallback";
      console.warn(
        "Warning: Using fallback method without proper password protection"
      );
    }
