...
async function extractTextFromPdf(inputPath: string, outputPath: string) {
  try {
    ...
    try {
      ...
      console.log(
        `Successfully extracted text using pdftotext to ${outputPath}`
      );
      return true;
      ...
      extractedText +=
        "PDF text content was extracted with limited formatting.\n";
      extractedText +=
        "For better results, try using the OCR option or convert to DOCX format.\n\n";
...
async function extractTextWithOCR(inputPath: string, outputPath: string) {
  try {
    ...
    throw new Error(
      "Failed to extract text with OCR: " +
        (error instanceof Error ? error.message : String(error))
    );
  }
}
  ...
  try {
    ...
    if (inputExt === "pdf") {
      if (process.platform === "win32") {
        ...
        await execPromise(
          `gswin64c -sDEVICE=${
            format === "jpg" || format === "jpeg" ? "jpeg" : "png16m"
          } -dNOPAUSE -dBATCH -dSAFER -r300 -dJPEGQ=${quality} -sOutputFile="${outputPath}" "${inputPath}"`
        );
      } else {
        // On Linux/Mac, try pdftoppm first
        try {
          const tempOutputPath = outputPath.substring(
            0,
          ...
          await execPromise(
            ...
            } -r 300 -jpegopt quality=${quality} -singlefile "${inputPath}" "${tempOutputPath}"`
          );
...
async function convertPdfToExcel(
  ...
  try {
    console.log(
      "Strategy 1: Attempting to use pdftotext for table extraction..."
    );
    const extractCommand = `pdftotext -table -csv "${inputPath}" "${join(
      tempDir,
      "extracted.csv"
    )}"`;
    ...
    try {
      ...
      if (existsSync(join(tempDir, "extracted.csv"))) {
        // Convert CSV to XLSX
        const csvToXlsxCommand = `libreoffice --headless --convert-to xlsx:"Calc MS Excel 2007 XML" --outdir "${tempDir}" "${join(
          tempDir,
          "extracted.csv"
        )}"`;
  ...
  try {
    ...
    const htmlFiles = (await readdir(tempDir)).filter((file) =>
      file.endsWith(".html")
    );
    if (htmlFiles.length > 0) {
      ...
      const xlsxFiles = (await readdir(tempDir)).filter((file) =>
        file.endsWith(".xlsx")
      );
      if (xlsxFiles.length > 0) {
        ...
        console.log(
          "Strategy 2 successful: Created XLSX from HTML intermediate"
        );
        return true;
      }
    }
...
async function handlePdfToExcelConversion(
  inputPath: string,
  outputPath: string,
  tempDir: string
): Promise<boolean> {
...
async function convertWithLibreOffice(
  ...
  try {
    ...
    if (
      ...
      console.log(
        `Using specific approach for PDF to ${format.toUpperCase()} conversion...`
      );
      ...
      try {
        ...
        // For PDF to XLSX - Using comprehensive multi-strategy approach
          ...
          console.log(
            "Using multi-strategy approach for PDF to XLSX conversion..."
          );
          ...
          try {
            ...
            const { stdout: htmlStdout, stderr: htmlStderr } =
              await execPromise(pdfToHtmlCommand);
            console.log("PDF to HTML conversion stdout:", htmlStdout);
            if (htmlStderr)
              console.error("PDF to HTML conversion stderr:", htmlStderr);
            ...
            if (htmlFiles.length > 0) {
              ...
              if (xlsxFiles.length > 0) {
                ...
                await copyFile(xlsxFile, outputPath);
                console.log(
                  "Strategy 1 successful: Created XLSX from HTML intermediate"
                );
          ...
          if (!conversionSuccess) {
            try {
              console.log(
                "Strategy 2: Trying direct PDF to XLSX conversion..."
              );
          ...
          if (!conversionSuccess) {
            try {
              ...
              const command = `pdftotext -table -csv "${tempInputPath}" "${join(
                tempDir,
                "extracted.csv"
              )}"`;
              ...
              try {
                ...
                if (existsSync(csvPath)) {
                  ...
                  // Check for output file
                  const xlsxFiles = (await readdir(tempDir)).filter(
                    (file) => file.endsWith(".xlsx") && file !== "input.xlsx"
                  );
                  if (xlsxFiles.length > 0) {
                    const xlsxFile = join(tempDir, xlsxFiles[0]);
                    await copyFile(xlsxFile, outputPath);
                    console.log(
                      "Strategy 3 successful: CSV to XLSX conversion worked"
                    );
                    conversionSuccess = true;
                  }
                }
          ...
          if (!conversionSuccess) {
            console.log(
              "All PDF to Excel conversion strategies failed. Creating a minimal XLSX file."
            );

            // Use direct library approach to create a minimal valid XLSX
            try {
              const inputBuffer = await readFile(tempInputPath);
              const outputBuffer = await libreConvert(
                inputBuffer,
                "xlsx",
                undefined
              );
        ...
        console.log(
          `Files in temp directory after conversion: ${files.join(", ")}`
        );

        const convertedFile = files.find(
          (file) => file !== "input.pdf" && file.endsWith(`.${format}`)
        );
        ...
        if (convertedFile) {
          ...
          console.log(
            `Successfully converted PDF to ${format.toUpperCase()} at ${outputPath}`
          );
        ...
        console.error(
          `PDF to ${format.toUpperCase()} specific conversion failed:`,
          pdfConversionError
        );
      }
      ...
      try {
        ...
        if (format === "docx" || format === "xlsx" || format === "pptx") {
          ...
          console.log(
            `Trying alternative PDF conversion command: ${alternativeCommand}`
          );
          ...
          if (convertedFile) {
            ...
            await copyFile(convertedPath, outputPath);
            console.log(
              `Successfully converted PDF to ${format.toUpperCase()} using alternative command at ${outputPath}`
            );
        ...
        console.error(
          `Alternative PDF to ${format.toUpperCase()} conversion failed:`,
          altError
        );
      }
    }
    ...
    if (
      (inputPath.toLowerCase().endsWith(".docx") ||
      ...
      try {
        ...
        const files = await readdir(tempDir);
        console.log(
          `Files in temp directory after DOCX to PPTX conversion: ${files.join(
            ", "
          )}`
        );
        ...
        console.error(
          "DOCX to PPTX specific conversion failed:",
          docxToPptxError
        );
      }
      ...
      try {
        ...
        if (existsSync(join(tempDir, "input.pdf"))) {
          ...
          if (pptxFile) {
            ...
            await copyFile(pptxPath, outputPath);
            console.log(
              `Successfully created PPTX using two-step conversion at ${outputPath}`
            );
    ...
    try {
      ...
      console.log(
        `Successfully converted with libreoffice-convert library to ${outputPath}`
      );
      ...
      console.log(
        "Direct library conversion failed, trying CLI approach:",
        libError
      );
    }
    ...
    try {
      ...
      console.log(
        `Executing standard conversion command: ${libreOfficeCommand}`
      );
      ...
      const convertedFile = tempFiles.find(
        (file) =>
      ...
      if (convertedFile) {
        ...
        console.log(
          `Successfully converted with standard LibreOffice command to ${outputPath}`
        );
      ...
      console.log(
        "Standard conversion command failed, trying alternative command...",
        error
      );
    }
    ...
    try {
      ...
      console.log(
        `Executing alternative conversion command: ${sofficeCommand}`
      );
      ...
      if (convertedFile) {
        ...
        await copyFile(convertedFilePath, outputPath);
        console.log(
          `Successfully converted with alternative command to ${outputPath}`
        );
      ...
      try {
        ...
        try {
          ...
          if (tabulaCheck) {
            ...
            const tabulaCommand = `tabula -o "${join(
              tempDir,
              "extracted.csv"
            )}" -p all "${tempInputPath}"`;
            ...
            if (existsSync(csvPath)) {
              ...
              if (xlsxFile) {
                ...
                console.log(
                  `Successfully converted to XLSX with tabula at ${outputPath}`
                );
        ...
        console.log(
          `Provided original PDF at ${outputPath.replace(".xlsx", ".pdf")}`
        );
        ...
        try {
          ...
          const tempFiles = await readdir(tempDir);
          const xlsxFile = tempFiles.find(
            (file) => file.endsWith(".xlsx") && file !== "empty.xlsx"
          );
          ...
          console.error(
            "Last resort Excel conversion failed:",
            lastResortError
          );
        }
      } catch (fallbackError) {
    ...
    throw new Error(
      `Failed to convert ${path.basename(inputPath)} to ${format.toUpperCase()}`
    );
  } catch (error) {
    console.error("LibreOffice conversion error:", error);
    throw new Error(
      "Failed to convert with LibreOffice: " +
        (error instanceof Error ? error.message : String(error))
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Processing file: ${file.name}, from ${inputFormat} to ${outputFormat}, size: ${fileSize} bytes`
    );
